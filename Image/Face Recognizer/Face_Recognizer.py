...
class Window:
	...
	def __init__(self,window):
		...
		self.win.geometry("635x469+399+188")
		...
		bFaceVid.grid(row =5,column=1,columnspan=6,padx = (30,10))
		
		bHelp = Button(self.win,text = "Help",width=8,font = ("Helvetica",12),command =self.help_cmd)
		bHelp.grid(row =6,column=1,columnspan=2)
		
		bExit = Button(self.win,text = "Exit",font = ("Helvetica",12),width=8,command = self.exit_cmd)
		bExit.grid(row =6,column=4,columnspan=2,padx = (150,0))
		
		bTrain = Button(self.win,text = "Train",font = ("Helvetica",12),width=8,command = self.train_cmd)
		bTrain.grid(row =6,column=3,columnspan=2,sticky='w',padx = (55,0))
		...
		try:
			...
			for x,y,w,h in faces:
				...
				try:
					...
					pass
			cv2.putText(img, text, (x, y-4), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 255), 2)
